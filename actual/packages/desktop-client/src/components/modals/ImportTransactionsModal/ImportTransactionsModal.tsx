...
export function ImportTransactionsModal({
  ...
  const [loadingState, setLoadingState] = useState<
    null | 'parsing' | 'importing' | 'classifying'
  >('parsing');
  ...
  async function mlUndo() {
    if (mlAction === 'confirmed') {
      for (const t of mlApplied) { try { await send('api/transaction-update', { id: t.id, fields: { category: null } }); } catch {} }
      void queryClient.invalidateQueries(payeeQueries.list());
    }
    setMlAction('pending');
    setMlApplied([]);
  ...
  return (
    ...
    >
      {({ state }) => (
        <>
          {!mlReviewPhase && loadingState !== 'classifying' && (
          <>
          <ModalHeader
            title={
              ...
              <View style={{ flexDirection: 'row', gap: 10, marginTop: 16, alignItems: 'center' }}>
                {mlPreds.length > 0 && mlAction === 'pending' && (
                  <>
                    ...
                    <Button variant="bare" onPress={() => mlRejectAll()} style={{ color: theme.errorText, border: '1px solid ' + (theme.errorText || 'red'), borderRadius: 4, padding: '6px 14px' }}>
                      {t('Reject All')}
                    </Button>
                  </>
                )}
                {mlPreds.length > 0 && mlAction === 'confirmed' && (
                  <>
                    ...
                    <Button variant="bare" onPress={() => void mlUndo()} style={{ textDecoration: 'underline', color: theme.pageTextLink }}>
                      {t('Undo')}
                    </Button>
                  </>
                )}
                ...
                <View style={{ flex: 1 }} />
                <Button variant="normal" onPress={() => { if (onImported) onImported(true); state.close(); }}>
                  {t('Close')}
                </Button>
              </View>
            </View>
          ...
          {loadingState === 'classifying' && (
            <View style={{ padding: 40, alignItems: 'center' }}>
              <Text style={{ fontSize: 14, color: theme.pageTextSubdued }}>
                {t('Classifying transactions with ML model...')}
              </Text>
            </View>
          )}
        </>
