...
def main() -> None:
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        with mlflow.start_run(run_name=f"{cfg['model']}-retrain"):
            ...
            production_accuracy = _production_accuracy(active_ref, X_val, y_val, label_classes)
            if production_accuracy is not None:
                mlflow.log_metric("production_accuracy", production_accuracy)
                mlflow.log_metric(
                    "accuracy_delta",
                    round(float(metrics["accuracy"]) - production_accuracy, 6),
                )

            candidate_ref = _candidate_reference(cfg, run_id, metrics, dataset_meta)
            promote = _should_promote(metrics["accuracy"], production_accuracy, cfg)

            if promote:
                _update_registry(cfg, registry, candidate_ref, registry_path)
                mlflow.set_tag("promotion_status", "promoted")
                mlflow.set_tag("promoted_model_uri", candidate_ref["model_uri"])
                print(f"[promotion] Promoted {candidate_ref['model_uri']} -> {registry_path}")
            else:
                mlflow.set_tag("promotion_status", "rejected")
                if active_ref:
                    mlflow.set_tag("kept_model_uri", str(active_ref.get("model_uri", "")))
                print(
                    "[promotion] Candidate kept out of production "
                    f"(candidate_accuracy={metrics['accuracy']:.4f}, "
                    f"production_accuracy={production_accuracy:.4f})"
                )

            print(
                f"\n[done] Retrain complete - accuracy={metrics['accuracy']:.4f} "
